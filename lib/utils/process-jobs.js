    ...
    async function runOrRetry() {
      if (self._processInterval) {
        ...
        if (jobDefinition.concurrency > jobDefinition.running && self._runningJobs.length < self._maxConcurrency) {
          ...
          // Check if job still exists in the database, if it doesn't it should't run
          // NOTE: Maybe this is not the best way to do this?
          let jobExists;
          try {
            jobExists = await self._collection.countDocuments({_id: job.attrs._id}) > 0;
          } catch (error) {
            // If there is an error with query retry
            jobProcessing();
            return;
          }

          if (!jobExists) {
            debug('[%s:%s] job no longer exists in db (probably canceled), freeing it up', job.attrs.name, job.attrs._id);
            self._lockedJobs.splice(self._lockedJobs.indexOf(job), 1);
            jobDefinition.locked--;
            jobProcessing();
            return;
